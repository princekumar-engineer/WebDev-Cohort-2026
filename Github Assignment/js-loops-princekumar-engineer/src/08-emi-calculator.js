...
export function calculateEMI(principal, monthlyRate, emi) {
  const FAIL = { months: -1, totalPaid: -1, totalInterest: -1 };

  // Validation
  if (
    typeof principal !== "number" || !isFinite(principal) || principal <= 0 ||
    typeof monthlyRate !== "number" || !isFinite(monthlyRate) || monthlyRate < 0 ||
    typeof emi !== "number" || !isFinite(emi) || emi <= 0
  ) {
    return FAIL;
  }

  // Infinite loop protection (first month check)
  ...
  while (remaining > EPSILON) {
    ...
    // Extra safety check (dynamic infinite protection)
    if (emi <= interest) {
      return FAIL;
    }
    ...
    // Safety cap (extreme edge case protection)
    if (months > 10000) {
      return FAIL;
    }
  }
