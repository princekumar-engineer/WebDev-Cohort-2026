...
export function railwayReservation(passengers, trains) {
  ...
  for (let i = 0; i < passengers.length; i++) {
    ...
    // Try preferred class
    if (
      matchedTrain.seats[preferred] !== undefined &&
      matchedTrain.seats[preferred] > 0
    ) {
      matchedTrain.seats[preferred]--;
      allocatedClass = preferred;
    }
    // Try fallback class
    else if (
      matchedTrain.seats[fallback] !== undefined &&
      matchedTrain.seats[fallback] > 0
    ) {
      matchedTrain.seats[fallback]--;
