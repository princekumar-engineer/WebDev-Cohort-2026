...
* `ON DELETE RESTRICT` prevents deleting a director if they have movies assigned to them.
...
* Junction table has foreign keys mapping to both main parent tables.
* Junction table can store additional custom payload properties (like `character_name`, `role_type`).
* `UNIQUE(film_id, actor_id)` prevents duplicating structural records.
