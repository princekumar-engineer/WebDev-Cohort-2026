# PostgreSQL(Version 17) Comprehensive Guide
...
TEXT              -- Unlimited length (highly optimized in Postgres)
...
TIMESTAMPTZ       -- Timestamp with timezone (Highly Recommended)
...
VALUES ('2026-12-31', '14:30:00', INTERVAL '2 hours');
...
```sql
JSON              -- Text-based JSON (stores exact copy, slow to query)
JSONB             -- Binary JSON (parsed on entry, fast to query, indexable)
...
VALUES ('{"user": "john", "action": "login", "status": "active"}');

-- Querying JSON text value
SELECT request_data->>'user' FROM api_logs;

-- Modifying a value inside JSONB
UPDATE api_logs 
SET request_data = jsonb_set(request_data, '{status}', '"inactive"') 
WHERE log_id = 1;
...
> **Modern Standard Note:** While `SERIAL` is a legacy shorthand, PostgreSQL modern best practice strongly recommends using the SQL standard `GENERATED ALWAYS AS IDENTITY`. It handles explicit inserts and permissions much more elegantly.

```sql
-- Legacy Shorthand (Implicit sequence):
CREATE TABLE users_legacy (
    user_id SERIAL PRIMARY KEY
);

-- Modern Standard (Highly Recommended):
...
-- Partial index (ideal for performance tuning)
...
1. **Prefer `GENERATED ALWAYS AS IDENTITY` over legacy `SERIAL` types.**
2. **Always add NOT NULL constraints to required fields** to save schema integrity.
3. **Create indexes on foreign keys and frequently queried columns** to ensure efficient joins.
4. **Use transactions (`BEGIN...COMMIT`)** for multiple operations affecting critical business data.
5. **Always use TIMESTAMPTZ over TIMESTAMP** to prevent daylight savings and server migration timezone bugs.
6. **Name constraints explicitly** (e.g., `CONSTRAINT chk_positive_price`) for easier debugging.
7. **Use VARCHAR over CHAR** unless a hard-set string format is strictly required.
8. **Automate regular database backups** using `pg_dump`.
9. **Use parameterized queries or prepared statements** to thoroughly block SQL Injection attacks.
10. **Keep PostgreSQL updated** to the latest minor versions to ensure performance and security patch compliance.
...
```sql
-- Terminate active connection backends first
...
-- Types
INT, INT GENERATED ALWAYS AS IDENTITY, VARCHAR(n), TEXT, DATE, TIMESTAMPTZ, BOOLEAN, JSONB
